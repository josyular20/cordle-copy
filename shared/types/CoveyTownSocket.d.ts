...
export type InteractableType = 'ConversationArea' | 'ViewingArea' | 'TicTacToeArea' | 'ConnectFourArea' | 'CordleArea';
...
/**
 * Type for the state of a Cordle game.
 * The state of the game includes the secret word, guesses made, and game status, and more
 */
export interface CordleGameState extends WinnableGameState { 
  // The playerID of the player1 player, if any
  player1?: PlayerID;
  // The playerID of the yellow player, if any
  player2?: PlayerID;
  // Whether the player1 player is ready to start the game
  player1Ready?: boolean;
  // Whether the player2 player is ready to start the game
  player2Ready?: boolean;
  // The number of the player who will make the first move
  firstPlayer: CordlePlayerNumber;
  // The secret word that the player needs to guess
  cordle: string | undefined;
  // The difficulty of the cordle game
  difficulty: CordleDifficulty | undefined
  // The current guesses made by the players
  guesses: ReadonlyArray<CordleGuess>;
  // The current state of the board with guesses evaluated
  guessesEvaluated: CordleCell[][];
}

/**
 * Type for a move in Cordle
 * The move includes the player who guesses, the guessed word, and the guess number
 */
export interface CordleGuess {
  // the player who made the guess
  player: PlayerID
  // The word guessed by the player
  guessedWord: string;
  // The guess number
  guessNumber: CordleGuessNumber;
}


export type CordleGuessNumber = 1 | 2 | 3 | 4 | 5 | 6;
export type CordleColor = 'Gray' | 'Yellow' | "Green";
export type CordleDifficulty = 'Easy' | 'Medium' | 'Hard';
export type CordlePlayerNumber = 'Player1' | 'Player2';
export type CordleCell = CordleColor | undefined;
...
export type InteractableCommand =  ViewingAreaUpdateCommand | JoinGameCommand | GameMoveCommand<TicTacToeMove> | GameMoveCommand<ConnectFourMove>| GameMoveCommand<CordleMove> | StartGameCommand | StartCordleGameCommand | LeaveGameCommand;
...
export interface StartCordleGameCommand {
  type: 'StartCordleGame';
  gameID: GameInstanceID;
  difficulty: CordleDifficulty;
}
